...
def print_searching_result(list_of_files):
	for file in list_of_files:
		print(file)
	print('Всего: ', len(list_of_files))

def search(random_files):
		search_string = str(input('Введите строку для поиска (для выхода введите q):'))
		if search_string != 'q':
				print_searching_result(get_searching_result(random_files, search_string))
				temp_files = []
				for file in get_searching_result(random_files, search_string):
